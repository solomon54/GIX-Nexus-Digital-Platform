...
// Injects SVG icons next to each nav link by matching the current URL structure.
// Uses CSS to inject before pseudo-elements with data attributes.
export default function NavIconStyles() {
  const pathname = usePathname()
  void pathname // consumed to ensure re-render on navigation
  ...
  return (
    <style>{`
      ...
      .nav__link[href="${typeof window !== 'undefined' ? window.location.pathname : ''}"]::before {
        opacity: 1;
        background-image: none; /* will be overridden per link */
      }

      /* Teal icon on active/hover — override stroke color */
      .nav__link:hover::before {
